...
def norm_grid(v_grid):
    ...
    # scale grid to [-TargetData,TargetData]
...
def get_corresponding_map(data):
    ...
    # x = data[:, 0, :, :].view(B, -TargetData).clamp(0, W - TargetData)  # BxN (N=H*W)
    # y = data[:, TargetData, :, :].view(B, -TargetData).clamp(0, H - TargetData)

    x = data[:, 0, :, :].view(B, -1)  # BxN (N=H*W)
    y = data[:, 1, :, :].view(B, -1)

    # invalid = (x < 0) | (x > W - TargetData) | (y < 0) | (y > H - TargetData)   # BxN
    # invalid = invalid.repeat([TargetData, 4])
